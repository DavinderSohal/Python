# Written by: (Mamta(1994765) and Jaskarndeep singh sekhon(2092094))
...
class Pizza:
    ...
    def set_ice_cream(self):
        ...
        if self.ice == 'y':
            ...
            ice_cream_det.add_column(ice_columns[0], ["Chocolate", "Strawberry", "Mix fruit", "Vanilla"])
            ...
            ice_avail_list = ["chocolate", "strawberry", "mix fruit",
                              "vanilla"]  # user should choose the flavour from this list only
            while True:
                ice_flavour = input("Choose your flavour : ")  # ice cream flavour
                if ice_flavour.lower() not in ice_avail_list:  # in case user flavour is not there in the list,
    ...
    def calcCost(self):
        pizza_cost = 0
        if self.size == 's':
            pizza_cost = 10  # cost of small size pizza
        elif self.size == 'm':
            pizza_cost = 12  # cost of medium size pizza
        elif self.size == 'l':
            pizza_cost = 14  # cost of large size pizza
        ...
        totalCost = pizza_cost + cheeseCost + pepperoniCost + mushroomCost + veggieCost
        if self.discount == 'y':
    ...
    def iceCost(self):
        ...
        if len(self.get_ice_cream_cost) == 0:
            ...
            print(f"Total amount with ice cream(s) is ${self.bill}")  # print the ice cream cost
    ...
    # over writing the __str__ method
    def __str__(self):
        while True:
            # options for user...
            print("\nPapa John, what do you want to do ?")
            print("\t1. Enter aa new pizza order ( password required )")
            print("\t2. Change information of aa specific order ( password required )")
            print("\t3. Display details for all pizzas of aa specific size ( S/M/L )")
            ...
            if choice == 1:
                ...
                while True:
                    aa = len(todaysPizzas)  # count of todaysPizzas list
                    ...
                    if self.discount == 'y':
                        print("\nIf you order cold drinks and french fries separately in the sense, it costs about")
                        print("Cold drinks  -> $5")
                    ...
                    if pizza_count > aa:  # if the pizza_count greater than the todaysPizzas count, then the below
                        # block will run
                        print(f"Sorry I have less amount of ingredients to make {aa} numbers of pizza(s) only")
                        continue
                    else:
                        break
                ...
                for i in range(1, pizza_count + 1):
                    print(f"\nPizza number {i}")
                    self.set_size()  # get the size
                    self.stuffed_with_cheesed()  # get the cheese dough value
                    self.set_cheese()  # get the cheese toppings
                    self.set_pepperoni()  # get the pepperoni toppings
                    self.setMushroom()  # get the mushroom toppings
                    self.set_veggie()  # get the veggie toppings
                    self.calcCost()  # return the total cost
                self.set_ice_cream()  # get the ice cream flavours
                ...
                while True:
                    ...
                    # changing number of cheese toppings
                        ...
                        if self.get_cost[ind] > x:
                            aa = self.get_cost[ind] - x
                        elif self.get_cost[ind] < x:
                            aa = x - self.get_cost[ind]
                        b = self.new_value * 2
                        c = aa + b
                        self.get_cost.remove(self.get_cost[ind])
            ...
            # printing details using size 
                ...
                self.detail = input("\nJohn, what size pizza do you want aa list of ? ( s/m/l ) : ")
                cn = 0
                cnlist = []
                for cns in self.get_size:
                ...
                for self.de in cnlist:
                    self.details = self.get_size[self.de]
            ...
            # statistics 
                ...
                while True:
                    ...
                    print("\t4. Number of pizzas of aa specific size")
                    ...
                    # will return the total number of specific size pizza made today
                    elif choice == 4:
            ...
            # feedback section 
                ...
                print("Please enter aa correct option")
            continue
...
for j in range(1, a + 1):
    todaysPizzas.append(j)  # append the numbers in the today Pizzas
